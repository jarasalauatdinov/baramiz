...
interface AppNavItem {
  to: string;
  label: string;
  icon: 'home' | 'explore' | 'route' | 'profile' | 'services';
  primary?: boolean;
}
...
export function AppBottomNav() {
  ...
  const items: AppNavItem[] = [
    ...
    { to: routePaths.appRouteBuilder, label: t('pages.appNav.route'), icon: 'route', primary: true },
    { to: routePaths.appServices, label: t('layout.navigation.services'), icon: 'services' },
    {
      to: routePaths.appProfile,
      label: t('pages.appNav.profile', { defaultValue: 'Profile' }),
      icon: 'profile',
    },
  ];
            ...
            <NavLink key={item.to} to={item.to} className={styles.bottomNavLink}>
              {({ isActive }) => (
                ...
                >
                  ...
                  <Text size="xs" fw={700} lh={1.1} className={styles.bottomNavLabel}>
                    {item.label}
                  </Text>
                </UnstyledButton>
              )}
