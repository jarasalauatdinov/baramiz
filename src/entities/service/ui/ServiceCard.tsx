...
import type { ServiceCardData, ServiceKind } from '../model/types';
...
export function ServiceCard({ item, actionLabel, kind, onBook, bookLabel }: ServiceCardProps) {
  const { t } = useTranslation();
  const resolvedKind = kind ?? 'experiences';
  const resolvedActionLabel = actionLabel ?? item.contactLabel ?? t('common.viewDetails');
  const resolvedBookLabel = bookLabel ?? t('pages.booking.common.bookNow');
  const coverage = item.availableCities && item.availableCities.length > 0
    ? item.availableCities.join(', ')
    : item.city;

  const contactButton = item.contactHref ? (
    <Button
      component="a"
      href={item.contactHref}
      target={item.contactHref.startsWith('http') ? '_blank' : undefined}
      rel={item.contactHref.startsWith('http') ? 'noreferrer' : undefined}
      variant={onBook ? 'subtle' : 'light'}
      color="forest"
      radius="xl"
      fullWidth
    >
    ...
    <Button variant={onBook ? 'subtle' : 'light'} color="forest" radius="xl" fullWidth>
      {resolvedActionLabel}
    </Button>
  );
  ...
  return (
    ...
    >
      <Stack justify="space-between" h="100%" gap="xl">
        ...
        {onBook ? (
          <Stack gap="xs">
            <Button color="sun" c="#2d2208" radius="xl" fullWidth onClick={onBook}>
              {resolvedBookLabel}
            </Button>
            {contactButton}
          </Stack>
