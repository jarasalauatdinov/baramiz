...
import { formatCategoryLabel, formatMinutesLabel } from '../lib/presentation';
  ...
  return (
    <Card
      ...
      style={{
        ...
        background: isImmersive ? publicUi.background.surfaceWarm : publicUi.background.surface,
        boxShadow: isImmersive ? publicUi.shadow.card : publicUi.shadow.cardSoft,
      }}
    >
      <Card.Section>
        <DestinationImage
        ...
        >
          ...
          >
            <Badge variant="filled" radius="xl" color="sun" c="#5a420b" style={{ backdropFilter: 'blur(8px)' }}>
              {place.city ?? t('common.unknownCity')}
            </Badge>
            {place.featured ? (
              <Badge variant="filled" radius="xl" color="forest" style={{ backdropFilter: 'blur(8px)' }}>
                {t('common.featured')}
              </Badge>
            ) : null}
          </Group>
      ...
      <Stack gap={isImmersive ? 'sm' : 'md'} p={{ base: 'md', md: 'xl' }} justify="space-between" style={{ flex: 1 }}>
        {isImmersive ? (
          <Text c="#6d6258" size="sm" style={{ lineHeight: 1.5 }} lineClamp={1}>
        ...
        <Button
          ...
          variant={isImmersive ? 'light' : 'light'}
          color={isImmersive ? 'sun' : 'forest'}
          c={isImmersive ? '#2d2208' : undefined}
          radius="xl"
          size={isImmersive ? 'sm' : 'sm'}
          fullWidth
          style={isImmersive ? { boxShadow: '0 10px 18px rgba(229, 182, 47, 0.12)' } : undefined}
        >
          {isImmersive
